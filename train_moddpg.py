# train_moddpg.py — Versi Final dengan Evaluasi, Plot, dan Penyimpanan Model

import numpy as np
import torch
from multi_env import MultiUAVEnv
from moddpg_agent import MODDPGAgent
import random
...
agents = [MODDPGAgent(i, OBS_DIM, ACT_DIM, N_AGENTS, {
    'device': device
}) for i in range(N_AGENTS)]
buffer = ReplayBuffer()
...
plt.title("Training Reward per Episode (MODDPG)")
plt.grid(True)
plt.savefig("reward_plot_moddpg.png")
plt.show()

# === Save Trained Actor Models ===
os.makedirs("saved_models/moddpg", exist_ok=True)

for i, agent in enumerate(agents):
    model_path = f"saved_models/moddpg/agent{i}_actor.pth"
